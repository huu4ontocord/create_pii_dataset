...
basic_regex = [
    ("EMAIL_ADDRESS"   , re.compile("([a-z0-9!#$%&'*+\/=?^_`{|.}~-]+@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)")),
    ...
    ("STREET_ADDRESS"  , re.compile('\d{1,4} [\w\s]{1,20} (?:street|st|avenue|ave|road|rd|highway|hwy|square|sq|trail|trl|drive|dr|court|ct|park|parkway|pkwy|circle|cir|boulevard|blvd)\W?(?=\s|$)')),
                        ...
                        if item[1] in ('GOVT_ID', 'STREET_ADDRESS', 'PERSON'):
                          if item[0] in public_figures:
                            ...
                            context[item[0]] = context.get(item[0], \
                                                      ...
                                                      faker_target_lang.ssn() if  item[1] == 'GOVT_ID' else \
                                                      faker_target_lang.address() if  item[1] == 'STREET_ADDRESS' else \
                                                      item[0])
                            if " " in item[0]:
      ...
      for l in tqdm(f):
        ...
        if [entity for entity in entities if entity.label_ == 'PERSON']:
          ...
          if len(ents) > 1 and len(ents) < 5:
            if swap or random.randint(0,1)==0 or '@' in sent or 'Social Sec' in sent or 'password' in sent:
              context = {}
              ents2 = []
              for item in ents:
                if item[1] in ('GOVT_ID', 'STREET_ADDRESS', 'PERSON'):
...
def do_translation(target_lang='hi', mariam_mt=True, person_swap=True):
  ...
  if mariam_mt:
    if mariam_mt_mapping.get(('en', target_lang)):
      model_name = mariam_mt_mapping.get(('en', target_lang))
      ...
      if person_swap:
        ...
        for item in ner2:
          ...
          if item[1] in ('COUNTRY', 'GPE', 'PERSON', 'GOVT_ID', 'STREET_ADDRESS',): #ORG
            ...
            text = text.replace(item[0], "*"+ str(_idx) + "*")
            context[item[0]] = context.get(item[0], \
                                          faker_target_lang.first_name()+" "+random.choice(bantu_surnames) if " " in item[0] and item[1] == 'PERSON' and target_lang in ('yo', 'sw') else \
                                          faker_target_lang.name() if " " in item[0] and item[1] == 'PERSON' else \
                                          faker_target_lang.first_name() if item[1] == 'PERSON' else \
